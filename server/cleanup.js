...
const { sweepDrops } = require('./handlers/item');
...
function startCleanupTasks(wss) {
  ...
  setInterval(() => {
    ...
    for (const player of players.values()) {
      ...
      if (Number.isFinite(mapId) && mapId > 0) activeRooms.add(roomOf(player));
    }
    for (const room of activeRooms) assignMapHost(room);
    for (const room of [...mapHosts.keys()]) {
      if (!activeRooms.has(room)) mapHosts.delete(room);
    }
  }, 5000);

  // Drop ledger: evaporate 3-minute-old drops, forget stale provisional ids
  setInterval(sweepDrops, 10000);
  ...
  setInterval(() => {
    ...
    for (const [id, player] of players.entries()) {
      if (now - player.lastUpdate > inactiveTimeout) {
        ...
        if (player.info) {
          broadcastToMap(player.mapId, { type: 'player_left', id }, id, player);
        }
