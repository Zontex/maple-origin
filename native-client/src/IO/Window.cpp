...
#else
		// Cursor clipping is a Windows nicety; GLFW has no portable
		// equivalent short of disabling the cursor entirely
		(void)window;
#endif
	}
...
#else
		std::snprintf(buf, sizeof(buf), "Icon.png");
#endif
...
#else
		// System mouse speed is a Windows-only tweak
		(void)sys_speed;
#endif
	}
