...
namespace ms
{
	// The v83 login sends a Windows HWID (MAC + volume serial). Our JSON
	// protocol has no such concept — a fixed placeholder keeps the
	// Configuration fields populated on non-Windows platforms.
	class HardwareInfo
	{
	public:
		HardwareInfo()
		{
			...
			char hwid[] = "0000000000";
			// Configuration::hwid() slices this into four 2-char pairs —
			// must be at least 8 chars
			char serial[] = "00000000";
			Configuration::get().set_macs(macs);
			Configuration::get().set_hwid(hwid, serial);
		}
	};
}
#else
