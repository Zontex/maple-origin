...
## Custom assets (ours, not Nexon's)
- **`public/data/custom-items.json` is a WZ overlay** grafted at load by `src/CustomWz.ts` (hook at the end of `WZManager._doLoad`): `wz["<file>.img"]` lists `$imgdir` nodes in the converter's JSON format, `names[id]` feeds `itemNames`/`itemDescs`. Consumers never know — Item.load, sitOnChair, icons, tooltips, remote seats all see ordinary nodes. Generated by scripts in `tools/custom-assets/` (edit the script, rerun it, rerun `tools/build-asset-manifest.js`); never hand-edit the JSON. Ids live in unused tails of real ranges (3019999) so the tab/category logic applies.
- **Roni's Developer Throne (3019999)**: the "GM RONI" throne — black velvet, gold lions, wings, purple aura — generated by Roni (`tools/custom-assets/throne_ai_original.png`, 1616x973 on black), cut out by border flood-fill (the velvet is as black as the background, so no brightness key), scaled to 160x131 with the flame tips under the base trimmed so the seat sits ~26px above the feet (`throne_source.png`); `throne.py` uses that file when present (its PIL-drawn chair is the fallback) and derives the icon from the crown, `info/devOnly=1` → only the superuser may sit (`useSetupItem` gate, "Only its maker may sit on this throne."), tradeBlock/notSale, 500 HP/MP per tick. Get it with `!item 3019999`. Chairs are drawn bottom-centred at the feet (`drawChair` ignores origin). **Chairs animate**: `sitOnChair` keeps every `effect/N` canvas in `chairFrames` and `updateChair` steps them by each frame's `delay` (remotes too) — Dragon Chair's 19 frames and Olivia's 16 used to freeze on frame 0. The throne's flames are 6 generated frames (`flame_frames` in throne.py: only purple pixels in the lower half are wave-displaced + brightness-flickered; the chair itself never moves), 120ms each. While seated, `Effects/DevThroneFx.ts` draws the cigar + smoke after the body using the composed frames' `brow` map point (mouth = brow + (±5, 14)); art rides the overlay's `fx` key.
