...
- **Roni's Developer Throne (3019999)**: the "GM RONI" throne — black velvet, gold lions, wings, purple aura — generated by Roni (`tools/custom-assets/throne_ai_original.png`, 1616x973 on black), cut out by border flood-fill (the velvet is as black as the background, so no brightness key), scaled to 160x131 with the flame tips under the base trimmed so the seat sits ~26px above the feet (`throne_source.png`); `throne.py` uses that file when present (its PIL-drawn chair is the fallback) and derives the icon from the crown, `info/devOnly=1` → only the superuser may sit (`useSetupItem` gate, "Only its maker may sit on this throne."), tradeBlock/notSale, 500 HP/MP per tick. Get it with `!item 3019999`. Chairs are drawn bottom-centred at the feet (`drawChair` ignores origin). While seated, `Effects/DevThroneFx.ts` draws the cigar + smoke after the body using the composed frames' `brow` map point (mouth = brow + (±5, 14)); art rides the overlay's `fx` key.
