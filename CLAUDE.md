...
- **Boot logos** (`src/LogoState.ts`, before LoginState on a cold boot; HMR auto-login skips them): `UI.wz/Logo.img/Wizet` (61 frames 550x420) to `BgmUI/WzLogo` (6.7s → 110ms/frame) then `Logo.img/Nexon` (frame 0 a white 800x600 card, then 136 frames 720x480 of the characters running across the logo) to `BgmUI/NxLogoMS` (9.5s → 70ms/frame; `NxLogo` is the bare 1.9s sting). Frames carry no `delay`. White screen; the Nexon frames are centred (their art is centred in the frame) but the Wizet card is drawn at the screen's TOP-LEFT — its logo sits in the card's bottom-right with a blank margin (bbox 252-538 x 214-412 of 550x420), so centring the card put the logo bottom-right of the screen. Click/Enter/Esc/Space skips one logo. The clips play once (`bgm.loop=false` after `playBackgroundMusic`); the login map's `Title` takes over.
...
- **737 items' `info/icon`/`iconRaw` are `uol` links** to another item's icon (every stat scroll points at its family's first: 2040502 → `../../02040002/info/icon`). `WZNode.nGetImage()`/`nPreloadImage()` follow a `uol`, and the inventory resolves it before the canvas check — an unresolved link drew nothing, leaving a bare id in the cell — and a ground drop built on the link (`DropItemSprite.frame`) had no width, drew at NaN and could never be picked up (`resolveIcon`).
...
## Kerning PQ (2026-08-22)
- `src/Events/KerningPQ.ts` — client-side event instance on the HenesysPQ pattern; Lakelis 9020000 / Nella 9020001 / Cloto 9020002 run Cosmic's scripts unmodified. Maps 103000800-804 (stages), 805 bonus, 890 exit; 30-min timer, bonus 60s (assumed); v83 rules 3-4 members lv 21-30. Stage exits are pt-5 `next00` portals gated in `MapleCharacter.enterPortal` via `KerningPQ.blocksPortal` (the kpq0-3 portal scripts' rule: `<n>stageclear` property). Stage clears relay to party members as `::kpq::<key>` party-chat lines (intercepted in `PartyManager.onPartyChat`); passengers keep `passengerProps`. Drops come from the regular table (Ligator 9300001 → Coupon 4001007 20%; Jr. Necki/Curse Eye/King Slime → Pass 4001008 100%, 6+3+1 = the 10 passes). Nella's rectangle stages use `Java.type('java.awt.Rectangle').contains(point)` — the shim now has it. **Solo test mode (superuser only)**: party of one eligible at any level, and on stages 2-4 two ghost members stand on correct spots (`ghostSpots`) so the 3-body puzzle is solvable alone — you must still stand on a correct one. Rewards list = Cosmic's setEventRewards; the wrong-answer art is `Effect.img/quest/party/wrong_kor`.

- **PQ sounds**: the stage/PQ clear voice is `Sound.wz/Field.img/Party1/Clear` and `Party1/Failed` plays on a wrong answer (showWrongEffect) and on a timed-out run — `Game.img/QuestClear` is the quest-completion jingle and wrong for party quests.
- **Event clock** (`Events/EventClock.ts`, both PQs): the "Time Left" plate is `Map.wz/Obj/etc.img/timer/backgrnd` (258x58; `backgrndhour` carries Hr/Min labels for ≥1h), digits are `etc.img/clock/fontTime` (26x35, `comma` is the colon) centred in the plate's black window (x 62..250, y 9..50), drawn top-centre. `etc.img/clock` itself has only the font — the plate is under `timer`.
