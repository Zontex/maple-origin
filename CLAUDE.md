...
- **Multi-hit skills stagger their lines by the body's frame delays** (`attackHitTimes`: line i lands when body frame trigger+i starts, the original client's `get_attackdelay`), every line calls `monster.hit(..., stackOffset)` so the numbers stack 24px a row (36 after a crit — `Monster.damageRowHeight`) and the mob's Damage thud + the skill's Hit clip play per line. Damage is rolled up front (Sacrifice drains off the total); deferred lines skip a mob that died or a map you left.
