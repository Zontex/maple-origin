...
## Debugging Tips
- **GM/dev chat commands** live in `src/DevCommands.ts` (`runDevCommand`, `!` prefix, checked in UIMap before `!text` falls through to guild chat; `!help` lists them): help, warp/map, level, item, meso, nx, job, hp, mp, heal, ap, sp, stat, maxstats, exp, fame, skill, maxskills, buff, dispel, killall/kill, spawn, cleardrops, pos, mapinfo, search. They act through the game's own APIs (inventory, stats, SkillManager, BuffManager, MapleMap.spawnMonster) so saves/relays follow; `!spawn` mobs are script-spawned (oId 300000+, this client only). Add a command = one entry in the `commands` table.
