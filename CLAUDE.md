...
- **Roni's Developer Throne (3019999)**: a from-scratch pixel-art chair (`tools/custom-assets/throne.py`, 72x96, drawn with PIL — crown, gold "R", code screen, crimson cushion), `info/devOnly=1` → only the superuser may sit (`useSetupItem` gate, "Only its maker may sit on this throne."), tradeBlock/notSale, 500 HP/MP per tick. Get it with `!item 3019999`. Chairs are drawn bottom-centred at the feet (`drawChair` ignores origin). While seated, `Effects/DevThroneFx.ts` draws the cigar + smoke after the body using the composed frames' `brow` map point (mouth = brow + (±5, 14)); art rides the overlay's `fx` key.
