...
### Fixed
- **Mobs froze until the backend was restarted** — the long-running one. Symptom: mobs stand still and ignore attacks while still dealing contact damage, meaning the client believes it is not the mob host (contact damage is the one part computed locally). Four distinct causes, found in order:
